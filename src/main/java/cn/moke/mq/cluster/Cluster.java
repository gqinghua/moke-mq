...
public class Cluster{
     ...
     */
    public static Set<Broker> getCustomerBrokerByQueues(String[] queueName){
    	Set<Broker> list = new HashSet<>();
    	if(null != queueName){
    		for(String name:queueName){
    			if(SLAVE_BROKER.containsKey(name)){
    				List<Group> groups = SLAVE_BROKER.get(name);
    				for(Group gp:groups){
    					if(gp.getMaster() != null){
    						list.add(gp.getMaster());
    					}
    				}
    	    	}
    		}
