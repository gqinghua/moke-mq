    	...
    	try {
	        ...
	        if (argsSize > 0) {
	        	...
	        	if(StringUtils.isNotBlank(info)){
	        		...
	        		if("all".equals(info)){
	        			...
	        			if(config.getEnableZookeeper()){
							...
							try {
								...
								ZkUtils.initZkClient(config.getZkConnect(), authString);
								StringWriter writer = new StringWriter();
								printTree(ZkUtils.zkClient.getZooKeeper(), "/uncodemq", writer, "\n");
								sb.append(writer.getBuffer().toString());
							} catch (Exception e) {
								e.printStackTrace();
							} 
	        			...
	        			if(config.getEnableZookeeper()){
							...
							try {
								...
								String[] list = getTree(ZkUtils.zkClient.getZooKeeper(), "/uncodemq/index");
								all.addAll(Arrays.asList(list));
								list = getTree(ZkUtils.zkClient.getZooKeeper(), "/uncodemq/producer");
								all.addAll(Arrays.asList(list));
								list = getTree(ZkUtils.zkClient.getZooKeeper(), "/uncodemq/consumer");
								all.addAll(Arrays.asList(list));
								list = getTree(ZkUtils.zkClient.getZooKeeper(), "/uncodemq/notspending");
								all.addAll(Arrays.asList(list));
								for (String pa : all) {
									try {
										ZkUtils.zkClient.delete(pa);
									} catch (Exception e) {
									}
								}
								StringWriter writer = new StringWriter();
	        		...
	        		}else if("amqno".equals(info)){
	        			if(config.getEnableZookeeper()){
	        				sb.append("zk:");
							try {
								String authString = config.getZkUsername() + ":" + config.getZkPassword();
								ZkUtils.initZkClient(config.getZkConnect(), authString);
								if(ZkUtils.zkClient.exists("/uncodemq/activemq")){
									ZkUtils.setData(ZkUtils.zkClient, "/uncodemq/activemq", "no");
								}
								StringWriter writer = new StringWriter();
								printTree(ZkUtils.zkClient.getZooKeeper(), "/uncodemq", writer, "\n");
								sb.append(writer.getBuffer().toString());
	        		...
	        		}else if("amqyes".equals(info)){
	        			if(config.getEnableZookeeper()){
	        				sb.append("zk:");
							try {
								String authString = config.getZkUsername() + ":" + config.getZkPassword();
								ZkUtils.initZkClient(config.getZkConnect(), authString);
								ZkUtils.makeSurePersistentPathExists(ZkUtils.zkClient, "/uncodemq/activemq");
								ZkUtils.setData(ZkUtils.zkClient, "/uncodemq/activemq", "yes");
								StringWriter writer = new StringWriter();
								printTree(ZkUtils.zkClient.getZooKeeper(), "/uncodemq", writer, "\n");
								sb.append(writer.getBuffer().toString());
							} catch (Exception e) {
					...
					nettyServer.registerHandler(RequestHandler.FETCH, new FetchRequestHandler(config));
