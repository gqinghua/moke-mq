...
import cn.moke.mq.store.TopicQueueIndex;
...
public class ZkTopicQueueReadIndex implements TopicQueueIndex{
    ...
    public static final String ZK_NOT_SPENDING_TOP_LIST = ZkUtils.ZK_MQ_BASE + "/notspending";
    ...
    public void sync2zk(int readNum, int readPosition, int readCounter, int writeNum, int writePosition, int writeCounter) {
    	...
    	if(Cluster.getCurrent() != null){
    		if(Cluster.getCurrent().getMaster().getHost().equals(Cluster.getMasterIps().peek())){
        		...
        		String notSpendingPath = ZK_NOT_SPENDING_TOP_LIST + "/" + Cluster.getCurrent().getMaster().getHost();
        		if(zkClient.exists(notSpendingPath)){
        			boolean change = false;
        			byte[] topicByte = zkClient.readData(notSpendingPath);
        			Set<String> topiclst = new HashSet<>();
        			if(topicByte != null){
        			...
        			if(topiclst.size() > 0){
        				if(readCounter < writeCounter){
        					if(!topiclst.contains(queueName)){
        						topiclst.add(queueName);
        						change = true;
        					}
            			}else{
            				if(topiclst.contains(queueName)){
        						topiclst.remove(queueName);
        						change = true;
        					}
            			}
        				if(change){
        					StringBuilder sb = new StringBuilder();
        					for(String tp:topiclst){
        						sb.append(tp).append(",");
        					}
        					if(sb.length()>0){
    							sb.deleteCharAt(sb.lastIndexOf(","));
    						}
        					zkClient.writeData(notSpendingPath, sb.toString().getBytes());
        				}
        			}else{
        				if(readCounter < writeCounter){
        					ZkUtils.makeSurePersistentPathExists(zkClient, notSpendingPath);
            				zkClient.writeData(notSpendingPath, queueName.getBytes());
        				}else{
        					zkClient.delete(notSpendingPath);
        				}
        			}
        		}else{
        			if(readCounter < writeCounter){
